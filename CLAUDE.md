...
9. **A PLATFORM YOU CANNOT ENTER FROM BELOW MUST PAY FOR ITSELF** (designer 2026-08-08). A ledge reachable only by dropping onto it, or only along one narrow guarded approach, is a good thing *when there is something on it to claim* — loot, a shop, Blompo, an NPC. That is what makes the player accept the narrow path with an enemy standing in it. **Empty, the same geometry is just a wall that looks like a route**, and the player spends Shift finding that out. So: when a platform's only entrance is awkward, either put a payoff on it or open a second way in. (Found on GenLevel8, where the designer placed a Blompo on such a platform themselves to fix it.)
...
⚠️ **TWO SEPARATE STAGES WERE EACH DARKENING DEEP ROCK, AND NEITHER KNEW ABOUT THE OTHER (fixed 2026-08-08).** `TileVariantGenerator.FlattenSprite` pulled every pixel toward the tile's **25th-percentile** luminance ("so bright brick faces don't drag it up"), which silently cost ~1.5× of brightness *before* `DeepTints` was applied. Stacked with a 0.66 tint and the 0.5 light, the core of a mass landed near 0.04 on screen. It now flattens toward the **mean**, which is brightness-preserving by construction — so the tint table is the only thing setting how dark deep rock gets, and can be tuned honestly. **When two stages both scale a value, make exactly one of them own it.**

⚠️ **`DepthFromAir` is a Chebyshev distance, so its contours are PERFECT RECTANGLES.** Every recession step landed on an axis-aligned box and the interior of a mass read as a black rectangle pasted onto the rock — the corners worst, because a right angle is the one shape stone never makes. The importer now nudges the depth by a hashed −1/0/+1 per cell so each boundary wanders a tile. Seed that hash differently from the tile-choice hash or the two correlate into a visible pattern. **Any distance-field-driven look needs its contour broken, or the geometry of the metric shows through.**
