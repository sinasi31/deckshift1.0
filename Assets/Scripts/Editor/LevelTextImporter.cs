...
public static class LevelTextImporter
{
    ...
    {
        "Ground Extra_153 Deep", "Ground Extra_185 Deep", "Ground Extra_101 Deep", "Ground Extra_49 Deep",
    };
            ...
            for (int row = 0; row < height; row++)
            {
                ...
                for (int col = 0; col < width; col++)
                {
                    ...
                    if (c == '#')
                    {
                        ...
                        // to flat black.
                        if (DepthFromAir(col, row, 3, IsSolid) >= 3)
                        {
                            TileBase deep = Resolve(DeepFillTiles[Mathf.Abs((col * 73856093) ^ (cellY * 19349663)) % DeepFillTiles.Length]);
                            if (deep != null)
                            {
