...
namespace Cainos.PixelArtMonster_Dungeon
{
    ...
    {
        //MOVEMENT PARAMETERS
        [FoldoutGroup("Params")] public MovementType defaultMovement = MovementType.Walk;
        [Space]
        [FoldoutGroup("Params")] public float walkSpeedMax = 2.5f;                           // max walk speed
        [FoldoutGroup("Params")] public float walkAcc = 10.0f;                               // walking acceleration
        [Space]
        [FoldoutGroup("Params")] public float runSpeedMax = 5.0f;                            // max run speed
        [FoldoutGroup("Params")] public float runAcc = 10.0f;                                // running acceleration
        [Space]
        [FoldoutGroup("Params")] public float airSpeedMax = 2.0f;                            // max move speed while in air
        [FoldoutGroup("Params")] public float airAcc = 8.0f;                                 // air acceleration
        [Space]
        [FoldoutGroup("Params")] public float groundBrakeAcc = 6.0f;                         // braking acceleration (from movement to still) while on ground
        [FoldoutGroup("Params")] public float airBrakeAcc = 1.0f;                            // braking acceleration (from movement to still) while in air
        [Space]
        [FoldoutGroup("Params")] public float jumpSpeed = 5.0f;                              // speed applied to character when jump
        [FoldoutGroup("Params")] public float jumpCooldown = 0.55f;                          // time to be able to jump again after landing
        [FoldoutGroup("Params")] public float jumpDelay = 0.5f;                              // time for the character to actually jump after jump key is pressed down, to make time for the jump prepare animation to be played
        [FoldoutGroup("Params")] public float jumpGravityMutiplier = 0.6f;                   // gravity multiplier when character is jumping, should be within [0.0,1.0], set it to lower value so that the longer you press the jump button, the higher the character can jump    
        [FoldoutGroup("Params")] public float fallGravityMutiplier = 1.3f;                   // gravity multiplier when character is falling, should be equal or greater than 1.0
        [Space]
        [FoldoutGroup("Params")] public Vector2 groundCheckSize = new Vector2(0.1f,0.1f);    // size of the box at the character's bottom to determine whether the character is on ground
        [FoldoutGroup("Params")] public float movingBlendTransitionSpeed = 2.0f;             // the transition speed of moving blend value
        [Space]
        [FoldoutGroup("Params")] public bool canAttackInAir = true;                          // can the character attack while in air?
        [FoldoutGroup("Params")] public bool canAttackWhenMoving = true;                     // can the character attack when moving? When turned off, it also forbit the character to move while in attack animation

        //RUNTIME INPUT PARAMETERS
        [FoldoutGroup("Runtime")] public Vector2 inputMove = Vector2.zero;                    // movement input, x for horizontal, y for vertical, x and y should be in [-1.0, 1.0]                 
        [FoldoutGroup("Runtime")] public bool inputMoveModifier = false;                      // switch between walk and run
        [FoldoutGroup("Runtime")] public bool inputJump = false;                              // jump input
        [FoldoutGroup("Runtime")] public bool inputAttack = false;                            // attack input


        [FoldoutGroup("Runtime"), ShowInInspector]                  // is the character dead? if dead, plays dead animation and disable control
        public bool IsDead
        {
            get { return isDead; }
            set
        ...
        public void Move(float inputH, bool inputRunning, bool inputJump)
        {
            ...
            if(Mathf.Abs(inputH) > 0.01f) pm.Facing = (PixelMonster.FacingType)Mathf.RoundToInt(inputH);
